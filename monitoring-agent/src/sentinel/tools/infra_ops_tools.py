"""Infrastructure operations tools for Sentinel.

Provides CDK and CloudFormation operations: synth/validate, diff, deploy,
stack status, drift detection, and rollback.

CDK/CLI operations are delegated to the Glitch agent (which can run them on
the owner's local machine via SSH) via invoke_glitch_agent.
...
from sentinel.aws_utils import REGION, CLIENT_CONFIG_LONG
...
async def cdk_synth_and_validate(stack_name: Optional[str] = None) -> str:
    ...
    Delegates to Glitch agent to run the commands on the owner's local machine via SSH.
    ...
    prompt = (
        f"Run this CDK synth command in the AgentCore-Glitch/infrastructure directory on the owner's "
        f"local machine (via SSH) and return the output: "
        f"npx cdk synth {stack_arg} 2>&1 | tail -50"
    )
    return await invoke_glitch_agent(prompt)
...
async def cdk_diff(stack_name: Optional[str] = None) -> str:
    ...
    Delegates to Glitch agent to run cdk diff on the owner's local machine via SSH.
    ...
    stack_arg = stack_name or ""
    prompt = (
        f"Run this CDK diff command in the AgentCore-Glitch/infrastructure directory on the owner's "
        f"local machine (via SSH) and return the full output: "
        f"npx cdk diff {stack_arg} 2>&1"
    )
    return await invoke_glitch_agent(prompt)
...
async def cdk_deploy_stack(stack_name: str, confirmed: bool = False) -> str:
    ...
    from sentinel.tools.glitch_invoke_tools import invoke_glitch_agent
    prompt = (
        f"Run this CDK deploy command in the AgentCore-Glitch/infrastructure directory on the owner's "
        f"local machine (via SSH) and return the output: "
