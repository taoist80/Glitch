...
def _db_unavailable_msg() -> str:
    return (
        "Protect DB is not yet reachable (still connecting). "
        "DB writes are retrying in the background. Try again in a moment."
    )


def _is_db_ready() -> bool:
    """Non-blocking check — returns True only if the pool is already established."""
...
async def protect_get_cameras() -> str:
    ...
    try:
        ...
        logger.error("protect_get_cameras error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_get_snapshot(
    ...
    try:
        ...
        logger.error("protect_get_snapshot error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_get_video_clip(
    ...
    try:
        ...
        logger.error("protect_get_video_clip error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_update_camera_settings(
    ...
    try:
        ...
        logger.error("protect_update_camera_settings error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_db_store_observation(
    ...
    """
    from glitch.protect.config import is_db_configured
    ...
    try:
        ...
        logger.error("protect_db_store_observation error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_db_query_patterns(
    ...
    try:
        ...
        logger.error("protect_db_query_patterns error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_db_get_baseline(
    ...
    try:
        ...
        logger.error("protect_db_get_baseline error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_db_record_alert(
    ...
    try:
        ...
        logger.error("protect_db_record_alert error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_db_update_pattern(
    ...
    try:
        ...
        logger.error("protect_db_update_pattern error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_analyze_event(event_id: str) -> str:
    ...
    try:
        ...
        logger.error("protect_analyze_event error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_should_alert(
    ...
    try:
        ...
        logger.error("protect_should_alert error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_send_telegram_alert(
    ...
    try:
        ...
        logger.error("protect_send_telegram_alert error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_get_alert_preferences(camera_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        logger.error("protect_get_alert_preferences error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_update_alert_preferences(
    ...
    try:
        ...
        logger.error("protect_update_alert_preferences error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_classify_snapshot(
    ...
    try:
        ...
        logger.error("protect_classify_snapshot error: %s", e, exc_info=True)
        return f"Error: {e}"
...
{{
  ...
  "summary": "..."
        ...
        logger.error("protect_detect_anomalies error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_start_monitoring(
    ...
    try:
        ...
        logger.error("protect_start_monitoring error: %s", e, exc_info=True)
        return f"Error: {e}"
...
async def protect_mark_false_positive(
    ...
    try:
        ...
        return json.dumps({
            ...
            "event_id": event_id,
            "reason": reason,
            "recommendation": "Run protect_learn_from_fps to apply corrections",
        })
    except Exception as e:
        return f"Error: {e}"
...
async def protect_track_entity(
    ...
    try:
        from glitch.protect import db as protect_db
        from glitch.protect.tracking import build_movement_path, format_path_summary
...
async def protect_configure_camera_topology(
    ...
    try:
        from glitch.protect import tracking
...
async def protect_privacy_export_entity_data(entity_id: str) -> str:
    ...
    try:
        from glitch.protect.privacy import export_entity_data
...
async def protect_privacy_delete_entity(entity_id: str, reason: str = "user_request") -> str:
    ...
    try:
        from glitch.protect.privacy import delete_entity_data
...
async def protect_privacy_run_retention_cleanup() -> str:
    ...
    """
    from glitch.protect.config import is_db_configured, is_protect_configured
    if not is_db_configured():
        return _db_not_configured_msg()

    try:
        from glitch.protect.privacy import run_retention_cleanup
        ...
        try:
            from glitch.protect.config import _get_ssm_parameter
...
# ============================================================
# LEARNING: On-demand learning / analysis tools
# ============================================================

@tool
async def protect_optimize_thresholds(lookback_days: int = 30) -> str:
    """Optimize per-camera alert anomaly-score thresholds using FP feedback.

    Performs a ROC-like sweep over the last N days of user-responded alerts and
    finds the threshold that minimises combined FP + miss rate. Auto-applies
    changes where improvement is significant and within safe bounds (±0.2).

    Args:
        lookback_days: How many days of alert history to analyse (default 30).

    Returns:
        JSON summary with cameras analysed, recommendations, and applied changes.
    """
    try:
        from glitch.protect.learning import optimize_alert_thresholds
        result = await optimize_alert_thresholds(lookback_days=lookback_days)
        return json.dumps(result, indent=2, default=str)
    except Exception as e:
        logger.error("protect_optimize_thresholds error: %s", e, exc_info=True)
        return f"Error: {e}"


@tool
async def protect_identify_regulars(
    min_sightings: int = 5,
    min_visits_per_week: float = 1.0,
    lookback_days: int = 30,
) -> str:
    """Identify unknown regular visitors and suggest trust-level promotions.

    Queries entities with trust_level='unknown' and enough sightings, runs
    entity-role classification, and returns candidates that exceed the confidence
    threshold. Does NOT auto-apply — use protect_classify_entity to confirm.

    Args:
        min_sightings: Minimum sighting count to consider an entity (default 5).
        min_visits_per_week: Minimum visit frequency (default 1.0).
        lookback_days: Look-back window in days (default 30).

    Returns:
        JSON with candidate entities and suggested roles/trust levels.
    """
    try:
        from glitch.protect.learning import identify_regular_visitors
        result = await identify_regular_visitors(
        ...
        logger.error("protect_identify_regulars error: %s", e, exc_info=True)
        return f"Error: {e}"


@tool
async def protect_daily_briefing() -> str:
    """Generate today's overnight security briefing (10pm–7am activity).

    Returns an assessment (ALL CLEAR / MONITOR / ELEVATED ACTIVITY / NEEDS
    ATTENTION) plus overnight event counts, alert counts, unknown/hostile entity
    sightings, and notable high-anomaly events.  Also stores the briefing in the
    DB (security_briefings table).

    Returns:
        JSON with assessment, formatted briefing message, and raw briefing data.
    """
    try:
        ...
        logger.error("protect_daily_briefing error: %s", e, exc_info=True)
        return f"Error: {e}"


@tool
async def protect_learn_from_fps(
    lookback_days: int = 7,
    auto_apply_threshold: float = 0.8,
) -> str:
    """Analyse recent false-positive alerts and auto-apply root-cause corrections.

    Identifies root causes (unknown regular visitor, threshold too low, incomplete
    baseline) and auto-applies corrections that exceed the confidence threshold.
    Lower-confidence suggestions are returned for user review.

    Args:
        lookback_days: Window of false-positive alerts to analyse (default 7).
        auto_apply_threshold: Minimum confidence to auto-apply a correction (default 0.8).

    Returns:
        JSON with root-cause analyses, applied corrections, and items pending review.
    """
    try:
        from glitch.protect.learning import learn_from_false_positives
        result = await learn_from_false_positives(
            lookback_days=lookback_days,
            auto_apply_threshold=auto_apply_threshold,
        )
        return json.dumps(result, indent=2, default=str)
    except Exception as e:
        logger.error("protect_learn_from_fps error: %s", e, exc_info=True)
        return f"Error: {e}"
...
EXTENDED_PROTECT_TOOLS = [
    ...
    # Group 15: Learning
    protect_optimize_thresholds,
    protect_identify_regulars,
    protect_daily_briefing,
    protect_learn_from_fps,
]
