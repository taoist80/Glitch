...
- SSH tools: ssh_list_hosts (list configured remote hosts), ssh_install_key(host_target, password) (install the project key on a new host using the user's password—e.g. user@hostname or user@hostname:port), ssh_run_command(host, command), ssh_read_file(host, remote_path), ssh_write_file(host, remote_path, content), ssh_mkdir(host, remote_path), ssh_file_exists(host, remote_path), ssh_list_dir(host, remote_path). Use these to connect to registered hosts, add new hosts (ssh_install_key then add to GLITCH_SSH_HOSTS), run commands, create files/directories, load MCP config or other config from the host, and store information. Always call ssh_list_hosts first to see which hosts are available. **IMPORTANT EXCEPTION: The Tailscale EC2 (glitch-tailscale / glitch.awoo.agency) must NEVER be accessed via SSH. It uses AWS SSM — use run_tailscale_ssm_command, run_tailscale_ensure_tls, or run_tailscale_renew_tls instead. Do not ask for SSH credentials for this host.**
