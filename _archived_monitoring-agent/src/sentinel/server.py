...
_poller: Optional["ProtectEventPoller"] = None
_processor: Optional["ProtectEventProcessor"] = None
...
async def _start_protect_subsystem() -> None:
    ...
    while DB connections, REST seed calls, and WS streams complete.
    """
    global _poller, _processor

    from sentinel.protect.poller import ProtectEventPoller
    from sentinel.protect.client import get_client as get_protect_client
    from sentinel.protect.config import get_protect_config, is_protect_configured

    # Resolve Protect config here (may make SSM/SecretsManager calls) so the
    # lifespan can yield immediately without blocking the health check.
    try:
    ...
    _component_health["protect_configured"] = configured
    if not configured:
    ...
    try:
        ...
        logger.warning("Failed to start event processor: %s — continuing without it", exc, exc_info=True)
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    """FastAPI lifespan: launch background Protect setup, clean up on shutdown.

    Yields immediately so AgentCore's health check can succeed before any
    SSM/SecretsManager calls or network connections are attempted.
    """
    asyncio.create_task(_start_protect_subsystem(), name="protect-startup")

    yield  # server runs here

    # Shutdown: stop processor, poller, close DB pool and httpx client.
