...
### Via Glitch Agent (invoke_glitch_agent)
- Nginx config issues → run `write-glitch-proxy-conf.sh` via SSM
- TLS cert issues → run `ensure-glitch-tls.sh` or `renew-glitch-tls.sh` via SSM
- EC2 service restart → systemctl commands via SSM
- File system issues → write/fix files via SSH

### Via Sentinel tools directly
...
Title: `fix: [brief description] (Sentinel auto-diagnosis)`
