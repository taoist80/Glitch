...
from sentinel.protect.config import parse_host_port
...
class ProtectEventPoller:
    ...
    async def _consume_websocket(self, stream_name: str, ws_path: str) -> None:
        ...
        probe_timeout = 20.0
        t1 = _time.monotonic()
        probe_url = f"https://{hostname}:{port}/proxy/protect/integration/v1/cameras"
        try:
            ...
            logger.warning(
                "ProtectEventPoller[%s] REST probe failed (%.0fms): %s — proceeding to WS (check firewall/port forward from AWS to %s:%s)",
                stream_name, (_time.monotonic() - t1) * 1000, probe_exc, hostname, port,
            )
