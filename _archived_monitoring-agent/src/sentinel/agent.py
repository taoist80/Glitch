...
- **UniFi Protect (13 core tools)**: cameras, events, snapshots, DB ops, alerts, monitoring controls, entity mgmt. Extended analytics/reporting available as library functions internally.
...
def _get_all_tools() -> List[Callable]:
    ...
    from sentinel.tools.protect_tools import CORE_PROTECT_TOOLS
    ...
    return [
        ...
        github_commit_file,
        github_create_pr,
        # Protect (13 core tools — extended tools are library functions, not LLM-visible)
        *CORE_PROTECT_TOOLS,
        # Pi-hole
        pihole_list_dns_records,
        pihole_add_dns_record,
...
class SentinelAgent:
    ...
    def __init__(self) -> None:
        ...
        from sentinel.tools.protect_tools import CORE_PROTECT_TOOLS
        from sentinel.tools.compound_tools import security_correlation_scan, analyze_and_alert
        n_protect = len(CORE_PROTECT_TOOLS)
        n_compound = len([security_correlation_scan, analyze_and_alert])
        ...
        logger.info(
            "SentinelAgent initialized with %d tools (%d protect, %d compound) and model %s",
            len(tools), n_protect, n_compound, model_id,
        )
